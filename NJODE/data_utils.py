...
from src.data_utils import get_treatment_indices
import torch
import socket
import copy
import pandas as pd
...
# TE-CDE/src/utils/data_utils.py data_to_torch_tensor
def cde_data_to_torch_tensor(data, sample_prop=1, time_concat=-1):
    """
    ...
    data_Time (torch tensor): containing time
    data_y (torch tensor): containing outcomes
    data_tr (torch tensor): containing current treatments
    treated_indicies (numpy array): containing treated indicies
    untreated_indicies (numpy array): containing untreated indicies
    """
    ...
    data_time = data["time_covariates"] 
    ...
    data_X = torch.from_numpy(data_x[sample_idxs:, 0:58, :time_concat])
    data_A = torch.from_numpy(data_a[sample_idxs:, :, :])
    data_Time = torch.from_numpy(data_Time[sample_idxs:, 0:58, :time_concat])
